...
#define LOCAL_HOST_IP_STRING "127.0.0.1"
#define PORT 8080
...
void FAkWaapiClient::Initialize()
{
    ...
    if (!m_AkWaapiClient)
    {
        ...
        FCoreDelegates::OnExit.AddLambda([]()
        {
            ...
            {
                m_AkWaapiClient->AppExitingCounter.Increment();
                TArray<uint64_t> aSubscriptionIDs;
                m_AkWaapiClient->m_wampEventCallbackMap.GetKeys(aSubscriptionIDs);
...
{
#if AK_SUPPORT_WAAPI
    return AppExitingCounter.GetValue() != 0;
#else
	return false;
#endif
}
...
{
#if AK_SUPPORT_WAAPI
    bIsConnectionClosing = isClosing;
#endif
}
...
void FAkWaapiClient::DeleteInstance()
{
    ...
    if (m_AkWaapiClient != nullptr)
    {
        ...
        m_AkWaapiClient->bIsConnectionClosing = true;
        m_AkWaapiClient->m_Client.Disconnect();
...
{
#if AK_SUPPORT_WAAPI
    return m_wampEventCallbackMap.Find(in_subscriptionId);
#else
	return nullptr;
#endif
}
...
bool FAkWaapiClient::IsProjectLoaded()
{
	...
	{
		pInstance->m_pConnectionHandler->ResetReconnectionDelay();
		return false;
	}

	return pInstance->iProjectLoaded.GetValue() == 1;
#else
	return false;
#endif
}
...
void FAkWaapiClient::ConnectionEstablished()
{
    ...
    if (m_AkWaapiClient != nullptr)
    {
        ...
        {
            iProjectLoaded.Set(CheckProjectLoaded() ? 1 : 0);
            if (iProjectLoaded.GetValue() == 1)
            {
...
{
#if AK_SUPPORT_WAAPI
    return m_Client.Connect(LOCAL_HOST_IP_STRING, PORT);
#else
	return false;
#endif
}
    ...
    uint64& out_subscriptionId, FString& out_result, int in_iTimeoutMs /*= 500*/)
{
    ...
    if (IsConnected())
    {
        if (!bIsConnectionClosing)
        {
            // Call for the AK WAAPI method using string params.
            {
                FScopeLock Lock(&ClientSection);
                eResult = m_Client.Subscribe(in_uri, TCHAR_TO_ANSI(*in_options), &WampEventCallbacks, out_subscriptionId, out_resultString, in_iTimeoutMs);
            }
            if (eResult)
            {
                m_wampEventCallbackMap.Add(out_subscriptionId, in_callback);
            }
            else
            {
    ...
    {
        m_pConnectionHandler->ResetReconnectionDelay();
    }
#endif
    return eResult;
}
...
bool FAkWaapiClient::Unsubscribe(const uint64_t& in_subscriptionId, FString& out_result, int in_iTimeoutMs /*= 500*/, bool in_bSilenceLog /*= false*/)
{
    ...
    if (IsConnected())
    {
        if (!bIsConnectionClosing)
        {
            ...
            {
                FScopeLock Lock(&ClientSection);
                eResult = m_Client.Unsubscribe(in_subscriptionId, out_resultString, in_iTimeoutMs);
            }
            if (eResult)
            {
                if (m_wampEventCallbackMap.Contains(in_subscriptionId))
                    m_wampEventCallbackMap.Remove(in_subscriptionId);
            }
...
bool FAkWaapiClient::Call(const char* in_uri, const FString& in_args, const FString& in_options, FString& out_result, int in_iTimeoutMs /*= 500*/, bool silenceLog /* = false*/)
{
    ...
    if (IsConnected())
    {
        if (!bIsConnectionClosing)
        {
            ...
            {
                FScopeLock Lock(&ClientSection);
                eResult = m_Client.Call(in_uri, TCHAR_TO_ANSI(*in_args), TCHAR_TO_ANSI(*in_options), out_resultString, in_iTimeoutMs);
            }
...
{
#if AK_SUPPORT_WAAPI
    m_pConnectionHandler = MakeShareable(new FAkWaapiClientConnectionHandler(*this));

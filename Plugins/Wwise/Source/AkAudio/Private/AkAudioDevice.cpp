    ...
    )
{
	if( EnsureInitialized() ) // ensure audiolib is initialized
	{
		if( AkBankManager != NULL && in_Bank)
		{
			...
			if (cbInfo)
			{
				return AK::SoundEngine::LoadBank(TCHAR_TO_AK(*(in_Bank->GetName())), AkAudioDeviceBankLoadCallback, cbInfo, in_memPoolId, out_bankID);
			}
		}
		else
		{
    ...
    )
{
	if ( m_bSoundEngineInitialized && in_Bank )
	{
		if( AkBankManager != NULL )
		{
			...
			if (cbInfo)
			{
				return AK::SoundEngine::UnloadBank(TCHAR_TO_AK(*(in_Bank->GetName())), NULL, AkAudioDeviceBankUnloadCallback, cbInfo);
			}
		}
		else
		{
	...
	)
{
	...
	if (m_bSoundEngineInitialized && in_pComponent && CallbackManager)
	{
		if (in_pComponent->VerifyEventName(in_EventName) && in_pComponent->AllowAudioPlayback())
		{
			...
			if (pPackage)
			{
				playingID = AK::SoundEngine::PostEvent(TCHAR_TO_AK(*in_EventName), gameObjID, in_uFlags | AK_EndOfEvent, &FAkComponentCallbackManager::AkComponentCallback, pPackage);
	...
	)
{
	...
	if (m_bSoundEngineInitialized)
	{
		...
		if (UsingSpatialAudioRooms(in_pListener->GetWorld()))
			eResult = AK::SpatialAudio::SetEmitterObstruction(emitterId, in_Obstruction);
		else
...
AKRESULT FAkAudioDevice::SetGeometry(AkGeometrySetID AcousticZoneID, AkTriangle* Triangles, AkUInt32 NumTriangles)
{
	AKRESULT eResult = AK_Fail;
	if (m_bSoundEngineInitialized)
	{
		eResult = AK::SpatialAudio::SetGeometry(AcousticZoneID, Triangles, NumTriangles);
	}
...
void FAkAudioDevice::CancelEventCallbackCookie( void* in_cookie )
{
	if ( m_bSoundEngineInitialized )
	{
		CallbackManager->CancelCallbackCookie( in_cookie );
	}
}

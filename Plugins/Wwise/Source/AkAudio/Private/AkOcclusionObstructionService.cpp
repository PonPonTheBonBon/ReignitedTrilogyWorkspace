...
void FAkListenerOcclusionObstructionPair::AsyncTraceFromSource(const FVector& SourcePosition, const FVector& EndPosition, int BoundingBoxPointIndex, ECollisionChannel CollisionChannel, UWorld* World)
{
	...
	{
		SourceTraceHandles[BoundingBoxPointIndex] = World->AsyncLineTraceByChannel(EAsyncTraceType::Single, SourcePosition, EndPosition, CollisionChannel);
	}
}
void FAkListenerOcclusionObstructionPair::AsyncTraceFromListener(const FVector& ListenerPosition, const FVector& EndPosition, int BoundingBoxPointIndex, ECollisionChannel CollisionChannel, UWorld* World)
{
	ensure(BoundingBoxPointIndex < NUM_BOUNDING_BOX_TRACE_POINTS);
	// Check that we're not stacking another async trace on top of one that hasn't completed yet.
	if (!World->IsTraceHandleValid(ListenerTraceHandles[BoundingBoxPointIndex], false))
	{
		ListenerTraceHandles[BoundingBoxPointIndex] = World->AsyncLineTraceByChannel(EAsyncTraceType::Single, ListenerPosition, EndPosition, CollisionChannel);
	}
}
...
void AkOcclusionObstructionService::Init(UAkComponent* in_akComponent, float in_refreshInterval)
{
	...
	if (AkAudioDevice && AkAudioDevice->UsingSpatialAudioRooms(in_akComponent->GetWorld()))
	{
		...
		{
			AK::SpatialAudio::SetEmitterObstruction(gameObjId, value);
		};
	}
	else
	{
...
void AkOcclusionObstructionService::Init(AAkAcousticPortal* in_portalId, float in_refreshInterval)
{
	...
	SetOcclusionObstructionFcn = [in_portalId](AkGameObjectID gameObjId, float value)
	{
		AK::SpatialAudio::SetPortalObstruction((AkPortalID)in_portalId, value);
	};
}
		...
		if (bNowOccluded)
		{
			...
			if (bAsync)
			{
				for (int PointIndex = 0; PointIndex < NUM_BOUNDING_BOX_TRACE_POINTS; ++PointIndex)
				{
					...
					MapEntry->AsyncTraceFromListener(ListenerPosition, Point, PointIndex, in_collisionChannel, CurrentWorld);
					MapEntry->AsyncTraceFromSource(SourcePosition, Point, PointIndex, in_collisionChannel, CurrentWorld);
				}
			}
			else
			{
